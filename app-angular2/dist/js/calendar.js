(function ($) {
	...
	AvailabilityCalendar.prototype = {
		...
		__createToolbar: function () {
			...
			$inputContainer.append('<input type="button" class="btn" title="This month" value="Mes Actual">');
			$inputContainer.append('<input type="button" class="btn" title="Previous month" value="&#10094;">');
			$inputContainer.append('<input type="button" class="btn" title="Next month" value="&#10095;">');
			$inputContainer.append('{{ hola }}');
		...
		__addEvents: function (/*cellIndexes, dates*/) {
			...
			var self = this;
			self.$cells.append('<div></div>');
			self.$cells.append('<div>0 Dobles</div>');
			...
			this.bookedDates.forEach(function (date) {
				if (date.start <= lastDate && date.end >= firstDate) {
					...
					if (startIndex !== undefined) {
						self.$cells.eq(startIndex).addClass('unavailable').append('<div class="first">hola</div>');
						++startIndex;
					}
					else {
